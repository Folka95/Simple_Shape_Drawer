...
void Ellipse_Polar_DrawingAlgorithm::draw(const Shape &shape, ScreenWriter *sw) const {
    ...
    if(a < 1.0 || b < 1.0) {
        std::cerr << "Ellipse_Polar_DrawingAlgorithm::draw : semi-axes must be greater than 0" << std::endl;
        return;
    }
